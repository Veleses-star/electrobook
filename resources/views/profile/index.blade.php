<x-app-layout>
    ...
    <div class="py-12 bg-kid-bg dark:bg-dark-bg">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                    ...
                    <div class="flex flex-col md:flex-row gap-8 items-center md:items-start">
                        ...
                        <div class="flex-1 text-center md:text-left">
                            <div class="flex flex-wrap items-center justify-center md:justify-start gap-2">
                                ...
                                @if($user->selected_status && isset($statusNames[$user->selected_status]))
                                    <span class="inline-block px-3 py-1 bg-yellow-100 dark:bg-yellow-900 text-yellow-700 dark:text-yellow-300 rounded-full text-sm font-bold">
                                        🏆 {{ $statusNames[$user->selected_status] }}
                                    </span>
                                @endif
                            </div>
            ...
            <!-- Купленные товары (с проверкой на существование item) -->
            <div class="bg-white dark:bg-dark-card rounded-2xl shadow-xl overflow-hidden">
                ...
                <div class="divide-y divide-gray-100 dark:divide-gray-700">
                    @forelse($purchases as $purchase)
                        @if($purchase->item)  <!-- защита от null -->
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p class="font-bold text-gray-800 dark:text-gray-100">
                                            {{ $purchase->item->type === 'status' ? ($statusNames[$purchase->item->name] ?? $purchase->item->name) : $purchase->item->name }}
                                        </p>
                                        <p class="text-xs text-gray-500 dark:text-gray-400">
                                            {{ $purchase->item->type === 'avatar' ? 'Аватар' : ($purchase->item->type === 'frame' ? 'Рамка' : ($purchase->item->type === 'status' ? 'Статус' : 'Тема')) }}
    ...
    @php
        // Массив для перевода английских названий статусов на русский (должен быть и в контроллере, но продублируем здесь)
        $statusNames = [
            'Youngster' => 'Молодец!',
            'Clever' => 'Умный',
            'Excellent' => 'Отличник',
