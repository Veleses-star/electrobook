<x-app-layout>
    ...
    <div class="py-12 bg-kid-bg dark:bg-dark-bg min-h-screen">
        <div class="max-w-6xl mx-auto px-4">
            @if(session('success'))
                <div class="bg-green-100 dark:bg-green-900 border-l-4 border-green-500 dark:border-green-400 text-green-700 dark:text-green-200 p-4 mb-6 rounded-xl">
                    {{ session('success') }}
                </div>
            @endif
            @if(session('error'))
                <div class="bg-red-100 dark:bg-red-900 border-l-4 border-red-500 dark:border-red-400 text-red-700 dark:text-red-200 p-4 mb-6 rounded-xl">
                    {{ session('error') }}
                </div>
            @endif

            @php
                // Группируем товары по типу, исключая frame (золотую рамку)
                $grouped = $items->where('type', '!=', 'frame')->groupBy('type');
            @endphp

            @foreach($grouped as $type => $typeItems)
                <div class="mb-12">
                    <h3 class="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-4 border-b-2 border-kid-secondary inline-block">
                        @if($type == 'avatar')
                            👤 Аватары
                        @elseif($type == 'theme')
                            🎨 Темы оформления
                        ...
                        @foreach($typeItems as $item)
                            @php
                                ...
                                if ($item->type === 'avatar') {
                                    ...
                                    // Для статусов – проверяем, есть ли у пользователя выбранный статус
                                    $isEquipped = (Auth::user()->active_status_id == $item->id);
                                }
                            @endphp
                                ...
                                <div class="p-6">
                                    ...
                                    <div class="mb-4">
                                        <span class="inline-block px-3 py-1 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 text-xs font-bold rounded-full">
                                            {{ $item->type === 'avatar' ? 'Аватар' : ($item->type === 'theme' ? 'Тема' : 'Статус') }}
                                        </span>
                                    </div>
                                    @if($isEquipped)
                                    ...
                                    @elseif($isPurchased)
                                        <form action="{{ route('shop.equip', $item->id) }}" method="POST">
                                            @csrf
                                            <button type="submit" class="w-full py-3 bg-kid-secondary dark:bg-dark-secondary text-black dark:text-white font-bold rounded-xl hover:bg-blue-400 dark:hover:bg-blue-500 transition-all shadow-lg hover:shadow-xl">
                                                Применить
                                            </button>
                                        </form>
                                    @else
                                        ...
                                        <form action="{{ route('shop.buy', $item->id) }}" method="POST">
                                            @csrf
                                            <button type="submit"
                                                    class="w-full py-3 bg-gradient-to-r from-orange-400 to-orange-500 text-black dark:text-white font-extrabold rounded-xl shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all {{ Auth::user()->points < $item->price ? 'opacity-50 cursor-not-allowed' : 'hover:from-orange-500 hover:to-orange-600' }}"
